...
public class AttivitaDAO {
    ...
    public List<Attivita> getAttivitaColtivatoreByCodFisc(String codFiscColtivatore) throws SQLException {
        ...
        final String SQL = "SELECT a.*, p.nome AS nomeProgetto, c.nome AS nomeColtura, c.idLotto AS idLottoColtura " +
                        "FROM Attivita a " +
                        "JOIN Progetto p ON a.idProgetto = p.idProgetto " +
                        "JOIN Coltura c ON a.idColtura = c.idColtura " +
                        "WHERE a.cod_fisc_coltivatore = ? " +
                        "ORDER BY a.dataInizio DESC";
                            
        try (Connection conn = DBManager.getConnection();
            PreparedStatement stmt = conn.prepareStatement(SQL)) {
                
            stmt.setString(1, codFiscColtivatore);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    coltivatore.setCod_fisc(codFiscColtivatore); 
                    Coltura coltura = new Coltura();
                    coltura.setIdColtura(rs.getString("idColtura"));
                    coltura.setNome(rs.getString("nomeColtura"));
                    
                    Progetto progetto = new Progetto();
                    progetto.setIdProgetto(rs.getString("idProgetto"));
                    progetto.setNome(rs.getString("nomeProgetto"));
            ...
            System.err.println("Errore DAO: Impossibile recuperare attività per il coltivatore " + codFiscColtivatore);
            throw e;
        } catch (IllegalArgumentException e) {
            throw new SQLException("Errore di mappatura ENUM nel DB.", e);
        }
    ...
    public List<Attivita> getAttivitaByColtivatoreCodFisc(String codFiscColtivatore) throws SQLException {
            List<Attivita> attivitaList = new ArrayList<>();
            
            // QUERY: Filtra l'Attivita sulla FK diretta (cod_fisc_coltivatore) e fa JOIN per contesto
            final String SQL = "SELECT a.*, p.nome AS nomeProgetto, c.nome AS nomeColtura, c.idLotto AS idLottoColtura " +
                            "FROM Attivita a " +
                            "JOIN Progetto p ON a.idProgetto = p.idProgetto " +
                            "JOIN Coltura c ON a.idColtura = c.idColtura " +
            ...
            try (Connection conn = DBManager.getConnection();
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        Coltivatore coltivatorePlaceholder = new Coltivatore(); 
                        coltivatorePlaceholder.setCod_fisc(codFiscColtivatore); 
                        
                        Progetto progettoPlaceholder = new Progetto();
                        progettoPlaceholder.setIdProgetto(rs.getString("idProgetto"));
                        progettoPlaceholder.setNome(rs.getString("nomeProgetto"));
                        
                        Coltura coltura = new Coltura();
                        coltura.setIdColtura(rs.getString("idColtura"));
                        coltura.setNome(rs.getString("nomeColtura"));
                        coltura.setIdLotto(rs.getString("idLottoColtura"));
                        
                        Attivita attivita = new Attivita(
                            rs.getString("idAttivita"),
                            rs.getString("nome"), // Nome Attività
                            rs.getObject("dataInizio", LocalDate.class),
                            rs.getObject("dataFine", LocalDate.class),
                            stato,
                            priorita,
                            rs.getDouble("costo"),
                            coltivatorePlaceholder,
                            coltura,
                            rs.getString("idProgetto")
                        );

...
public class ProgettoDAO {
    ...
    public List<Progetto> getProgettiByColtivatoreCodFisc(String codFiscColtivatore) throws SQLException {
        ...
        final String SQL = "SELECT p.*, " +
                           ...
                           "u.nome AS nomeProprietario, u.cognome AS cognomeProprietario " +
                           "FROM Progetto p " +
                           "JOIN ColtivatoreProgetto cp ON p.idProgetto = cp.idProgetto " + // Filtro M:N
                           "JOIN Lotto l ON p.idLotto = l.idLotto " + 
                           "JOIN Utente u ON p.cod_fisc_proprietario = u.cod_fisc " + // Per il nome del proprietario
                           "WHERE cp.cod_fisc_coltivatore = ?";
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            System.err.println("Errore DAO: Impossibile recuperare i progetti per il coltivatore " + codFiscColtivatore);
            throw e;
        } catch (IllegalArgumentException e) {
            throw new SQLException("Errore di mappatura ENUM Stagione nel DB.", e);
        }
